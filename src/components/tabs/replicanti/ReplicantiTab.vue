...
export default {
  ...
  methods: {
    update() {
      ...
      this.multDT = Decimal.max(Decimal.log10(Replicanti.amount).times(getAdjustedGlyphEffect("replicationdtgain")), 1);
      ...
      this.nextEffarigRGThreshold = new Decimal(Number.MAX_VALUE).pow(Effarig.bonusRG.add(2));
      this.canSeeGalaxyButton = Replicanti.galaxies.max.gte(1) || PlayerProgress.eternityUnlocked();
    ...
    calculateEstimate() {
      ...
      const logGainFactorPerTick = player.replicanti.chance
        .add(1)
        .ln()
        .mul(updateRateMs)
        .mul(getGameSpeedupForDisplay())
