...
export const TD = {
  ...
  basePurchase: {
    ...
    isActive: dim => (dim
      ? ImaginaryUpgrade(14).canBeApplied || (dim === 8 && Decimal.gt(GlyphInfo.time.sacrificeInfo.effect(), 1))
  ...
  timeGlyphSacrifice: {
    ...
    multValue: () => (TimeDimension(8).isProducing
      ...
      : DC.D1),
    isActive: () => Decimal.gt(GlyphInfo.time.sacrificeInfo.effect(), 1),
    ...
    multValue: dim => {
      ...
      if (EternityChallenge(9).isRunning) {
        allMult = allMult.times(
          Currency.infinityPower.value.max(1).pow(Decimal.div(InfinityDimensions.powerConversionRate, 7))
  ...
  glyph: {
    name: "Glyph Effects",
    powValue: () => Decimal.mul(getAdjustedGlyphEffect("timepow"), getAdjustedGlyphEffect("effarigdimensions")),
  ...
  alchemy: {
    ...
    multValue: dim => Decimal.pow(AlchemyResource.dimensionality.effectOrDefault(1),
      dim ? 1 : MultiplierTabHelper.activeDimCount("TD")),
    powValue: () => Decimal.mul(AlchemyResource.time.effectOrDefault(1), Ra.momentumValue),
  ...
  nerfCursed: {
    ...
    isActive: () => Decimal.neq(getAdjustedGlyphEffect("curseddimensions"), 1),
