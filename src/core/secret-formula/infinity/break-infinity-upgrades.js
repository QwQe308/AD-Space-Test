...
export const breakInfinityUpgrades = {
  ...
  achievementMult: {
    ...
    get description() {
      if (PlayerProgress.imaginaryUnlocked())
        return "Antimatter Dimensions gain a multiplier based on total Abyss Research Nodes Researched";
      return "Antimatter Dimensions gain a multiplier based on Achievements completed";
    },
    effect: () =>
      (PlayerProgress.imaginaryUnlocked()
        ? Math.max(Math.pow(AbyssResearches.all.filter(x => x.level.gte(1)).length, 3) / 10, 1)
  ...
  slowestChallengeMult: {
    ...
    effect: () => Decimal.clampMin(new Decimal(3000000).div(Time.worstChallenge.totalMilliseconds.clampMin(33)), 1)
      .pow(1.33),
    formatEffect: value => formatX(value, 2, 2),
    hasCap: true,
    cap: DC.D3E4,
  ...
  tickspeedCostMult: rebuyable({
    ...
    afterEC: () =>
      (EternityChallenge(11).completions > 0 ? `After EC11: ${formatX(Player.tickSpeedMultDecrease, 2, 2)}` : ""),
    noLabel: true,
  ...
  dimCostMult: rebuyable({
    ...
    afterEC: () =>
      (EternityChallenge(6).completions > 0 ? `After EC6: ${formatX(Player.dimensionMultDecrease, 2, 2)}` : ""),
    noLabel: true,
